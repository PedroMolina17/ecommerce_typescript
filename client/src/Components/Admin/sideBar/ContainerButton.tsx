...
const ContainerButton = ({ title, name, onClick, children }: Props) => {
  ...
  return (
    <div
      onClick={onClick}
      className={`${
        selectNav === name ? "bg-dark text-primary " : "text-white"
      } hover:bg-darkw h-12 overflow-hidden flex items-center gap-1 cursor-pointer rounded-bl-md  rounded-tl-md relative`}
    >
